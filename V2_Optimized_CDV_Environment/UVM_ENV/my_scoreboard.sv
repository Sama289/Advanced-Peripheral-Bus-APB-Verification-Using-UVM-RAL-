...
uvm_analysis_export #(apb_sequence_item) sco_ap;
            ...
            if(APB_vif.pwrite) begin
                Assoc_Array[seq_item.paddr] = seq_item.pwdata;
                 ...
                 if(seq_item.prdata == Assoc_Array[seq_item.paddr]) begin
                     `uvm_info("RIGHT DATA" , $sformatf("rdata = 0x%0h , Expec_rdata = 0x%0h", seq_item.prdata,Assoc_Array[seq_item.paddr]) , UVM_LOW);          
                 end
                 else begin
                    //`uvm_info("WRONG DATA" , $sformatf("rdata = 0x%0h , Expec_rdata = 0x%0h", seq_item.prdata,Assoc_Array[seq_item.paddr]) , UVM_LOW);          
                    `uvm_error("WRONG DATA" , $sformatf("rdata = 0x%0h , Expec_rdata = 0x%0h", seq_item.prdata,Assoc_Array[seq_item.paddr]));         
